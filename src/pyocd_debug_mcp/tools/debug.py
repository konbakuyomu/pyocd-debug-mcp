...
def wait_halt(timeout: float = 30.0, resume_first: bool = True) -> dict:
    """Wait for the target to halt (e.g., hit a breakpoint).

    This is the key "wait" mechanism for AI debugging:
    1. Optionally resumes target execution
    2. Polls target state until HALTED
    3. Returns with register context when halted

    Args:
        timeout: Maximum seconds to wait (default 30).
        resume_first: Whether to resume target before waiting (default True).
    """
    target = session_mgr.target
    ...
    while True:
        ...
        if state == Target.State.HALTED:
            ...
            # Try to identify halt reason from DFSR
            try:
                dfsr = target.read32(SCB_DFSR)
                if dfsr & (1 << 0):
...
def sample_variable(
    ...
    interval: float = 0.5,
    count: int = 200,
    halt_on_read: bool = False,
) -> dict:
    """Periodically sample a memory location (e.g., a global variable).

    Reads a variable at a fixed address every `interval` seconds,
    collecting `count` samples. The target should be RUNNING during sampling
    unless halt_on_read is True.

    Args:
        address: Memory address of the variable.
        size: Variable size (1, 2, or 4 bytes).
        interval: Seconds between samples (default 0.5).
        count: Number of samples to collect (default 200).
        halt_on_read: If True, halt→read→resume each sample (slower but safer).
    """
    target = session_mgr.target
    samples = []
    start_time = time.monotonic()

    read_fn = {1: target.read8, 2: target.read16, 4: target.read32}.get(size)
    if read_fn is None:
        raise ValueError(f"Unsupported size: {size}. Use 1, 2, or 4.")

    for i in range(count):
        try:
            if halt_on_read:
                was_running = target.get_state() != Target.State.HALTED
                if was_running:
                    target.halt()
            ...
            if halt_on_read and was_running:
                target.resume()
